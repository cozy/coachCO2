...
export const sendBikeGoalMeasuresForAccount = async (client, account) => {
  ...
  logService(
    'info',
    `Query commute trips with: ${JSON.stringify(bikeCommuteQuery)}`
  )
  const timeseries = await client.queryAll(bikeCommuteQuery)
  logService('info', `Number of commute trips on bike: ${timeseries?.length}`)
  if (timeseries?.length < 1) {
    logService('info', 'No timeseries found. Abort.')
    return null
  }
  ...
  logService(
    'info',
    `Number of days with commute bike : ${daysWithBikeCommute}`
  )
